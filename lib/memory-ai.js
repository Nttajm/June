...
Default action is WRITE. Bias toward storing a memory with a low recallScore rather than returning empty arrays.
Industry pattern: extract durable facts from every turn; control spam via recallScore + retrieval, not by refusing to store.

DO save (even from short / casual turns):
...
- Any named song, artist, show, game, book, creator → media (0.25–0.55 unless they gush)
...
- People, pets, places, plans with continuity value → invent a category if needed

knownInterestTitles / knownGeneralInfo mean UPSERT (merge new detail into that title) — never treat "already known" as a reason to return empty unless the new turn adds zero new information beyond an exact ack.

DO skip ONLY true one-token acknowledgements with no entity and no new fact ("yeah", "ok", "mhm", "lol", silence). A short reply that names anything durable ("yeah Drake", "sure jazz", "maybe Seattle", "I'm at work") MUST produce at least one generalInfo or categorized entry.
...
5. Prefer upserting an existing title over creating near-duplicates — but still WRITE the upsert when there is any new detail, status, or confirmation.
...
12. Store what the USER said about their life. Do NOT store June's guessed/public answers as user facts. If she speculated an outline or explanation, that speculation is not a memory.
...
If nothing new to store but the user made a real request, still set chatTitle and chatSummaryHint.
Use the pure-filler template ONLY for bare acknowledgements with no durable content ("yeah", "ok", "mhm") — never for name/language/location/work/media/people/place facts, and never because a title already exists in knownInterestTitles.
...
export async function analyzeUserIntent({ userText, memory, context, history = [], sessionPaused = false, onUsage = null }) {
  ...
  const response = await client.chat.completions.create({
    ...chatModelOptions(config.memoryAiModel, { maxTokens: 80, temperature: 0.2 }),
    messages: [
  ...
  if (usage) {
    try {
      onUsage?.({ agent: "memory", model: config.memoryAiModel, usage });
    } catch {}
  }
...
export async function consolidateSessionMemory({ history, memory, existingDirectory, onUsage = null }) {
  if (!client) return null;
  ...
  const response = await client.chat.completions.create({
    ...chatModelOptions(config.memoryAiModel, { maxTokens: 500, temperature: 0.3 }),
    messages: [
...
export async function deduplicateMemories(memory) {
  ...
  const response = await client.chat.completions.create({
    ...chatModelOptions(config.memoryAiModel, { maxTokens: 400, temperature: 0.2 }),
    messages: [
