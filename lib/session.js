...
import { createTTS, ttsAvailable, getAvailableProviders, stripStallMarkers } from "./tts.js";
...
export class VoiceSession {
  ...
  #emitDelta(gen, delta) {
    ...
    const gapped = injectAutoGaps(gen, clean);
    if (!gapped) return;

    gen.fullGappedText = (gen.fullGappedText || "") + gapped;

    const displayText = stripStallMarkers(gapped);
    this.send({ type: "assistant_delta", text: displayText, textWithStalls: gapped, turnId: gen.id });
    
    if (gen.ttsCtl) {
      gen.ttsCtl.push(gapped);
    } 
    
    if (!gen.speaking && displayText) {
      gen.speaking = true;
  ...
  #onTtsError(err) {
    ...
    this.tts = null;
    const gen = this.gen;
    if (gen?.llmDone && gen.ttsCtl && !gen.finalized) {
      gen.ttsCtl.cancel?.();
      gen.ttsCtl = null;
      this.#finalize(gen, { speakFallback: true });
    }
  ...
  #finalize(gen, { speakFallback = false } = {}) {
    ...
    const flushed = flushAutoGaps(gen);
    if (flushed) {
      gen.fullGappedText = (gen.fullGappedText || "") + flushed;
      const displayFlushed = stripStallMarkers(flushed);
      this.send({ type: "assistant_delta", text: displayFlushed, textWithStalls: flushed, turnId: gen.id });
      if (gen.ttsCtl) gen.ttsCtl.push(flushed);
    }

    const textWithStalls = gen.fullGappedText || "";
    const spoken = stripStallMarkers(textWithStalls);
