...
import { getUserName, getGeneralFacts, flattenInterestHooks } from "./memory-store.js";
...
export function shouldRefreshSnapshot(currentCache, newTranscript, history = []) {
  ...
  // Only the latest utterance + last 2 turns count as "still on this topic".
  // Using a wider window kept old shows alive forever after the chat moved on.
  const recent = [
    newTranscript || "",
  ...
  const topicMentioned = recent.includes(currentTopicKey.slice(0, 8));
  if (topicMentioned) return false;

  // Topic left the live window — refresh so follow-ups can re-anchor.
  return true;
}

export async function runSnapshotAgent({ transcript, history = [], memory = {}, signal, onUsage = null } = {}) {
  ...
  const knownFacts = [
    ...getGeneralFacts(memory).slice(0, 4).map((g) => ({ title: g.title, content: g.content })),
    ...flattenInterestHooks(memory, 8).map((h) => ({ title: h.title, category: h.category })),
  ];
  ...
  try {
    const response = await client.chat.completions.create(
      {
        ...chatModelOptions(config.snapshotAiModel, { maxTokens: 450, temperature: 0.4 }),
        messages: [
    ...
    result = normalizeResult(parseJsonObject(extractChatText(response)));
  } catch (err) {
