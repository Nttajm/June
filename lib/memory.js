...
const GENERIC_UTTERANCE_RE = /^(fine|good|okay|ok|yeah|yep|mhm|uh huh|nothing|not much|idk|i don't know|hey|hi|hello|sup|nm|cool|sure|thanks|thank you|yo)\.?$/i;
...
export function buildConversationRhythm(userText = "", history = [], recentCallbacks = []) {
  const lines = [];
  ...
  if (isGenericUtterance(userText)) {
...
export function buildMemoryEngagement(memory, { userText = "", history = [], recentCallbacks = [] } = {}) {
  ...
  const interests = semantic.filter(s => s.category !== 'fact');

  if (interests.length === 0 && logs.length === 0) return '';

  const lines = [];
  lines.push('--- MEMORY ENGAGEMENT (remember them — but don\'t fixate) ---');
  lines.push('Memory is seasoning, not the whole meal. ONE detail every few turns — not every reply.');
  lines.push('A show or video they mentioned once is NOT their entire personality. Do not keep interviewing them about it.');
  lines.push('When they talk about something NEW, follow their current topic. Never redirect back to an old show/video.');
  lines.push('Do NOT default to bland openers like "what\'s on your mind?", "how can I help?", "oh gotcha", "oh sorry", or "my bad".');
  lines.push('Jump straight into reaction or substance. No acknowledgment preamble.');

  if (recentCallbacks.length > 0) {
    lines.push('');
    lines.push(`SESSION RULE — already callback'd (DO NOT repeat): ${recentCallbacks.join(', ')}`);
    lines.push('Pick a DIFFERENT memory detail, or skip memory entirely this turn.');
  }

  const cleanUser = (userText || '').trim();
  if (cleanUser && isGenericUtterance(cleanUser)) {
    lines.push('');
    lines.push('Short/generic reply — teasing or matching energy is fine. Memory callback is optional, not required.');
  }

  const rotated = pickRotatedInterest(memory, recentCallbacks);
  if (rotated && recentCallbacks.length > 0) {
    lines.push('');
    lines.push(`If using memory this turn, prefer something OTHER than what you already brought up — e.g. ${rotated.subject}: ${rotated.value}`);
  }

  if (interests.length > 0) {
    lines.push('');
    lines.push('Things you know (pick ONE occasionally — rotate, don\'t fixate):');
    for (const sem of interests.slice(0, 10)) {
      const used = recentCallbacks.some(v => matchesTopic(v, sem.value));
      lines.push(`- ${sem.subject}: ${sem.value}${used ? ' (already used — skip unless they bring it up)' : ''}`);
    }
  }
  ...
  if (recent.length > 0) {
    lines.push('');
    lines.push('Recent passing stuff (use sparingly):');
    for (const log of recent) {
      const used = recentCallbacks.some(v => matchesTopic(v, log.value));
      lines.push(`- ${log.subject}: ${log.value}${used ? ' (already used)' : ''}`);
    }
  }

  lines.push('');
  lines.push('Good: mention their show once early, then drop it until they do.');
  lines.push('Bad: asking about the same show/video every turn because it\'s the only thing in memory.');
...
export function buildMemoryThoughtCache(memory, { recentCallbacks = [] } = {}) {
  ...
  const semantic = memory.semantic || [];
  const logs = memory.logs || [];
  const interests = semantic.filter(s => s.category === 'interest' || s.category === 'preference');
...
export function mergeThoughtCache(asyncCache, memory, { recentCallbacks = [] } = {}) {
  ...
  return {
    ...sync,
    ...asyncCache,
    confidence: recentCallbacks.length >= 2
      ? Math.min(asyncCache.confidence || 0, 0.45)
      : Math.max(sync.confidence, Math.min(asyncCache.confidence || 0, 0.55)),
    memoryBridge: recentCallbacks.length >= 2 ? null : (asyncCache.memoryBridge || sync.memoryBridge),
    juneSelfDrop: asyncCache.juneSelfDrop || sync.juneSelfDrop,
    casualDrops: [...new Set([...(asyncCache.casualDrops || []), ...(sync.casualDrops || [])])].slice(0, 2),
...
export function injectAutoGaps(gen, freshCleanText) {
  gen.autoGapBuffer = (gen.autoGapBuffer || "") + freshCleanText;
  let out = "";
  let i = 0;
  let buf = gen.autoGapBuffer;

  while (i < buf.length) {
    if (buf[i] === '.') {
      let dots = 1;
      let j = i + 1;
      while (j < buf.length && buf[j] === '.') { dots++; j++; }
      
      if (dots >= 3) {
        out += buf.substring(i, j) + "{-gap:0.4-}";
        i = j;
      } else if (j === buf.length) {
        if (!gen.hasSentFirstWord) {
          out += buf.substring(i, j);
          i = j;
        } else {
          break;
        }
      } else {
        if (dots === 1 && j < buf.length && /[0-9]/.test(buf[j])) {
          out += ".";
          i++;
        } else {
          out += buf.substring(i, j) + "{-gap:0.3-}";
          i = j;
        }
      }
    } else if (buf[i] === '-') {
      let dashes = 1;
      let j = i + 1;
      while (j < buf.length && buf[j] === '-') { dashes++; j++; }
      ...
      if (/[a-zA-Z0-9]/.test(buf[i])) gen.hasSentFirstWord = true;
      out += buf[i];
      i++;
    }
  }

  gen.autoGapBuffer = buf.slice(i);
  return out;
}

export function flushAutoGaps(gen) {
  let out = gen.autoGapBuffer || "";
  gen.autoGapBuffer = "";
  if (!out) return "";
  out = out.replace(/\.+$/, (m) => m.length >= 3 ? m + "{-gap:0.4-}" : m + "{-gap:0.3-}");
  out = out.replace(/-+$/, (m) => m.length >= 2 ? m + "{-gap:0.3-}" : m);
  return out;
}
